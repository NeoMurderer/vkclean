...
function setFilter(_this) {
	...
	switch (elem) {
      case "add":
        chrome.storage.sync.set({"add": type});
        break;
      case "apps":
        chrome.storage.sync.set({"apps": type});
        break;
      case "link":
        chrome.storage.sync.set({"link": type});
        break;
      case "vkclean":
        chrome.storage.sync.set({"vkclean": type});
        break;
      case "short_link":
        chrome.storage.sync.set({"short_link": type});
        break;
      case "repost":
        chrome.storage.sync.set({"repost": type});
        break;
    }
}
function getFilter () {
			...
			$(".text-block").append(
				$("<li/>",{"text":list[i],"id":i}).prepend(
					$("<span/>").click(function () {
						...
						list.splice(i, 1);
						console.log(list);
						chrome.storage.sync.set({"text": JSON.stringify(list)},function  () {
							_this.slideUp(500);
						});
					})
				)
